...
var request = {};
	...
	CommonUtil.executeAjax("0200021",request,function(data) {
		...
		var accessNum = data.responseBody.accessNum;
		var customNum = data.responseBody.customNum;
		if (activityList&&activityList.length>0) {
			...
			for (var key in activityList) {
				...
				html += '<p class="p_txt_gray">访问客户数：<span>'+code.accessNum+'</span>    客户数：<span>'+code.customNum+'</span></p>';
				html += '</div>';
			...
			$("#activAct").text(total);
			$("#visitAct").text(accessNum);
			$("#customerAct").text(customNum);
			$("#activeDiv").append(html);
			myScroll.refresh();
		} else {
			promt("未加载到赠险信息");
		}
