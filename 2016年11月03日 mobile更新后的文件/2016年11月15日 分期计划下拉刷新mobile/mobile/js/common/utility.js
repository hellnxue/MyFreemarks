...
//获取登陆MVC参数
function getJosonParam(queryString,type){
	// 分离参数对 ?key=value&key2=value2
	var parameters = queryString.split("&amp;quot;");
	var pos, paraName, paraValue;
	for (var i = 0; i < parameters.length; i++) {
		// 获取等号位置
		pos = parameters[i].indexOf(type);
		if (pos == -1) { continue; }
	    // 获取name 和 value
	    paraName = parameters[i];
	    paraValue =  parameters[i+2];
	    return paraValue;
	}
}

// type=encrypt 加密  其他解密
function getEncriptParams(param,type){
	var url = "/emploan/credit/encriptParams";
	var result = null;
	$.ajax({
		url: url,
		data:{encryptData:param,type:type},
		type:"post",
		cache: false,
		async: false,
		success:function(data){
